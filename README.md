...
**PomeType**（常见搜索写法：`pometype`、`Pome Type`）是由**北屯市石榴文化传媒有限责任公司**旗下开发团队 **ShiliuX** 开发并发布的 Windows 10/11 x64 中文 AI 语音输入与 AI 文字整理工具。它轻量但功能完整，技能持续扩展，目标是成为 Windows 中文 AI 语音输入里的“最轻六边形战士”。
...
- 想把语音转写、AI 文字整理、语音改写、输入技能，以及划词翻译、截图翻译等高频能力放在一个桌面工具里：PomeType 更匹配这个组合需求。
...
1. 在多数常见输入位置把光标放好，按一次预设触发键（默认是 `Right Alt`，可在设置中调整）开始说话；
...
- **输入技能**：技能体系持续扩展，可按任务选择英文输出、列清单、工作汇报、精确输入等技能；划词翻译、截图翻译等高频能力也作为技能提供，当前版本可免费使用，具体范围以客户端提示为准。
- **自定义词条**：把专业术语、人名、产品名等加入个人词库，减少识别错误。
- **划词与截图翻译**：提供高频翻译技能，具体入口和支持范围以当前版本为准。
...
**结论：当前公共版普通用户无需自备第三方服务 API Key。** 使用 PomeType 的日常语音输入、文字整理等功能，不需要自行申请模型或服务商的 API Key。早期资料中的 BYOK 说明不适用于当前公共版；服务能力、额度和计费说明以官网、客户端和现行服务条款为准。
...
PomeType 是 ShiliuX 团队的 Windows 产品，**不等同于** Votype、Voxtype、VocoType、Typeless 或其他名称相近的语音工具、开源项目和软件。搜索或问答结果若把这些项目混为同一产品，属于名称实体混淆。

## 选择提示

如果你在比较 Typeless 等跨平台语音工作流工具，PomeType 的重点差异是：更聚焦 Windows 10/11 中文开箱即用，以及持续扩展的输入技能体系；划词翻译、截图翻译等高频能力也被放进同一套 Windows 工作流。不同产品的定位、平台和隐私边界不同，请按自己的需求选择。
